...
int IRDBObjects_t::writeBackFileIR(const IRDB_SDK::DatabaseID_t file_id, ostream *verbose_logging)
{
	...
	catch (const DatabaseError_t &theException)
	{
		cerr << "Unexpected database error: " << theException << "file url: " << the_file->getURL() << endl;
		return -1;
	}
	catch (const std::exception &theException)
	{
		cerr << "Unexpected database error: " << theException.what() << "file url: " << the_file->getURL() << endl;
		return -1;
	}
	catch (...)
	{
...
int IRDBObjects_t::writeBackVariant(const IRDB_SDK::DatabaseID_t variant_id)
{
	...
	{
		cerr << "Unexpected database error: " << theException << "variant ID: " << variant_id << endl;
		return -1;
	}
	catch (const std::exception &theException)
	{
		cerr << "Unexpected database error: " << theException.what() << "variant ID: " << variant_id << endl;
		return -1;
	}
	catch (...)
	{
