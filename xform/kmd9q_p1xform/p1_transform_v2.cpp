...
int main(int argc, char **argv)
{
  if(argc!=3)
    {
      cerr<<"Usage: [the executable] <vid> [p1.candidates]"<<endl;
      exit(-1);
    }
  ...
  try {
	...
	fprintf(stderr, "P1: Looking at function: %s size: %d\n", func->GetName().c_str(), undoList.size());
	...
	if (!rewriteFunction)
	  {
	    fprintf(stderr,"P1: %d instructions to rollback for function %s\n", undoList.size(), func->GetName().c_str());
	    undo(undoList);
	  }
	else {
  ...
  cout << "List of functions transformed:" << endl;
