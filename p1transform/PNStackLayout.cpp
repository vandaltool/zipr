...
PNStackLayout::PNStackLayout(StackLayout stack_layout, IRDB_SDK::Function_t* func) : stack_layout(stack_layout)

{
	ALIGNMENT_BYTE_SIZE=IRDB_SDK::FileIR_t::getArchitectureBitWidth()/sizeof(int);
...
void PNStackLayout::AddRandomPadding(bool isaligned)
{
	...
	unsigned int curpad = this->GetRandomPadding(mem_objects[0]->getSize());
	total_padding += curpad;
	mem_objects[0]->SetPaddingSize(curpad + mem_objects[0]->GetPaddingSize());
...
} // end of PNStackLayout::AddRandomPadding()

//Basically this is the same process as the random padding function, but the padding
