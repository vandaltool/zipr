...
create_table()
{
	$PEASOUP_HOME/tools/db/pdb_create_program_tables.sh $1 $2 $3 $4 $5 $6 $7 db.tmp.$$
...
update_file_info()
{
	...
	psql -q -t -c "UPDATE file_info SET orig_file_id = '$FILE_ID', address_table_name = '${pn}_address', function_table_name = '${pn}_function', instruction_table_name = '${pn}_instruction', icfs_table_name = '${pn}_icfs', icfs_map_table_name= '${pn}_icfs_map', relocs_table_name = '${pn}_relocs', types_table_name = '${pn}_types' WHERE file_id = '$FILE_ID';" || exit 1
	...
	$SECURITY_TRANSFORMS_HOME/bin/meds2pdb ${fn}.annot ${fn}.infoannot $FILE_ID ${pn}_function ${pn}_instruction ${pn}_address ${pn}_types  ${pn}_icfs ${pn}_icfs_map $fn ${fn}.STARSxrefs || exit 1
}
