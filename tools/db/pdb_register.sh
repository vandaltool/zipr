...
create_table()
{
	$PEASOUP_HOME/tools/db/pdb_create_program_tables.sh $1 $2 $3 $4 $5 $6 db.tmp.$$
...
update_file_info()
{
	...
	psql -q -t -c "UPDATE file_info SET orig_file_id = '$FILE_ID', address_table_name = '${pn}_address', function_table_name = '${pn}_function', instruction_table_name = '${pn}_instruction', ibtargets_table_name = '${pn}_ibtargets', relocs_table_name = '${pn}_relocs', types_table_name = '${pn}_types' WHERE file_id = '$FILE_ID';" || exit 1
	...
	create_table ${pn}_address ${pn}_function ${pn}_instruction ${pn}_ibtargets ${pn}_relocs ${pn}_types
