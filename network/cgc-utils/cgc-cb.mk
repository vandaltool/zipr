...
LD_ELF                  = /usr/bin/ld
ZIPR                    = /techx_share/techx_umbrella/peasoup/peasoup_examples/tools/ps_analyze_cgc.cb-testing.sh
...
get_cb_bins   		= $(sort $(filter-out %_zipr, $(filter-out %_partial, $(filter-out %_patched, $(notdir $(wildcard $(BIN_DIR)/$(EXE)*))))))
get_cb_zipr_bins	= $(sort $(filter-out %_partial, $(filter-out %_patched, $(notdir $(wildcard $(BIN_DIR)/$(EXE)*_zipr)))))
#get_cb_patched_bins = $(sort $(filter-out %_partial, $(filter %_patched, $(notdir $(wildcard $(BIN_DIR)/$(EXE)*)))))
get_cb_patched_bins = $(sort $(filter-out %_zipr, $(filter-out %_partial, $(filter %_patched, $(notdir $(wildcard $(BIN_DIR)/$(EXE)*))))))
...
make_elf: prep $(RELEASE_PATH)
ifdef DO_COVERAGE
	echo "Making the elf directory."
	rm -rf $(BIN_DIR)/elf
	mkdir $(BIN_DIR)/elf
	cp $(BIN_DIR)/* $(BIN_DIR)/elf/ || true
	cgc2elf $(BIN_DIR)/elf/*
else
ifdef DO_TRACE
	echo "Making the elf directory."
...
ziprd: prep $(RELEASE_PATH)
ifdef DO_ZIPR
	echo "Running ziprd target."
	$(ZIPR) $(RELEASE_PATH) $(RELEASE_PATH)_zipr $(EXTRA_PS)
endif
...
ifdef DO_TRACE
	if [ -d poller/for-release ]; then _CB_SERVER_TRACE_OUTPUT_DIR=../../build/for-release-traces/ cb-test $(call get_test_args) --xml_dir poller/for-release/ --directory $(BIN_DIR)/elf --log $(BUILD_DIR)/$(RELEASE_EXE).trace-for-release.txt --cb $(call get_cb_bins) --pcap $(PCAP_FILE_PATH) --timeout 3600 --wrapper /usr/share/cgc-pin/wrappers/trace_wrapper.sh; fi
endif
	if [ -d poller/for-release ]; then cb-test $(call get_test_args) --xml_dir poller/for-release/ --directory $(BIN_DIR) --log $(BUILD_DIR)/$(RELEASE_EXE).for-release.txt --cb $(call get_cb_bins) --pcap $(PCAP_FILE_PATH); fi
ifdef DO_ZIPR
	if [ -d poller/for-release ]; then cb-test $(call get_test_args) --xml_dir poller/for-release/ --directory $(BIN_DIR) --log $(BUILD_DIR)/$(RELEASE_EXE).zipr-for-release.txt --cb $(call get_cb_zipr_bins) --pcap $(PCAP_FILE_PATH); fi
endif
...
# Polls generated from fuzzing
	if [ -d poller/for-coverage ]; then cb-test $(call get_test_args) --xml_dir poller/for-coverage/ --directory $(BIN_DIR) --log $(BUILD_DIR)/$(RELEASE_EXE).coverage-for-release.txt --cb $(call get_cb_bins); touch $(BUILD_DIR)/$(RELEASE_EXE).coverage-for-testing.txt; fi

# Polls that the user generated from AFL.
ifdef DO_COVERAGE
	if [ -d poller/for-coverage ]; then _CB_SERVER_TRACE_OUTPUT_DIR=../../build/for-coverage-traces/ cb-test $(call get_test_args) --xml_dir poller/for-coverage/ --directory $(BIN_DIR)/elf --log $(BUILD_DIR)/$(RELEASE_EXE).trace-for-coverage.txt --cb $(call get_cb_bins) --pcap $(PCAP_FILE_PATH) --timeout 3600 --wrapper /usr/share/cgc-pin/wrappers/trace_wrapper.sh; fi
endif

# Polls that the CB author intends to NOT release the resulting network traffic during CQE
ifdef DO_TRACE
	if [ -d poller/for-testing ]; then _CB_SERVER_TRACE_OUTPUT_DIR=../../build/for-testing-traces/ cb-test $(call get_test_args) --xml_dir poller/for-testing/ --directory $(BIN_DIR)/elf --log $(BUILD_DIR)/$(RELEASE_EXE).trace-for-testing.txt --cb $(call get_cb_bins) --timeout 3600 --wrapper /usr/share/cgc-pin/wrappers/trace_wrapper.sh; fi
endif
...
ifdef DO_ZIPR
	if [ -d poller/for-testing ]; then cb-test $(call get_test_args) --xml_dir poller/for-testing/ --directory $(BIN_DIR) --log $(BUILD_DIR)/$(RELEASE_EXE).zipr-for-testing.txt --cb $(call get_cb_zipr_bins); fi
endif
...
check-ids:
	if [ -d poller/for-release ]; then cb-test $(call get_ids_args) $(call get_test_args) --enable_remote --xml_dir poller/for-release/ --directory $(BIN_DIR) --cb $(call get_cb_patched_bins); fi
	if [ -d poller/for-testing ]; then cb-test $(call get_ids_args) $(call get_test_args) --enable_remote --xml_dir poller/for-testing/ --directory $(BIN_DIR) --cb $(call get_cb_bins); fi
...
check-remote:
	if [ -d poller/for-release ]; then cb-test $(call get_test_args) --enable_remote --xml_dir poller/for-release/ --directory $(BIN_DIR) --log $(BUILD_DIR)/$(RELEASE_EXE).for-release-remote.txt --cb $(call get_cb_patched_bins); fi
	if [ -d poller/for-testing ]; then cb-test $(call get_test_args) --enable_remote --xml_dir poller/for-testing/ --directory $(BIN_DIR) --log $(BUILD_DIR)/$(PATCHED_EXE).for-release-remote.txt --cb $(call get_cb_bins); fi
	cb-test $(call get_test_args) --enable_remote --xml_dir $(POV_DIR) --directory $(BIN_DIR) --log $(BUILD_DIR)/$(RELEASE_EXE).pov-remote.txt --failure_ok --cb $(call get_cb_patched_bins)
	cb-test $(call get_test_args) --enable_remote --xml_dir $(POV_DIR) --directory $(BIN_DIR) --log $(BUILD_DIR)/$(PATCHED_EXE).pov-remote.txt --failure_ok --should_core --cb $(call get_cb_bins)
...
ifeq ($(strip $(BINS)),)
build: prep release patched ziprd make_elf pov build-partial
else
