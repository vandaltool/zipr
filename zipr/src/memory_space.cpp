...
void ZiprMemorySpace_t::MergeFreeRange(RangeAddress_t addr)
{
	...
	if(IsValidRange(itp1))
	{
		 ...
		 */
		free_ranges.erase(itp1);
	} 
	else if(IsValidRange(itm1))	 // not addr+1 is still busy, so we don't merge against it.
	{
		r=*itm1;
...
{
	for(auto r : free_ranges.byAddress())
	{
...
{
	return it!=free_ranges.byAddress().end();
}
...
{
	const auto range=FindFreeRange((RangeAddress_t)-1);
...
std::list<Range_t> ZiprMemorySpace_t::getFreeRanges(size_t size)
{
	...
	for(auto r : free_ranges.bySize(size))
	{
		if(r.getSize() >=  size)
			result.push_back(r);
	}
...
{
	if(size == 0) size++;
	auto v = vector<Range_t>();
	for(auto r : free_ranges.bySize(size))
	{
		assert(r.getSize() >= (unsigned) size);
		v.push_back(r);
