		...
		do
			...
			case $config in
				...
				zafl_untracer_critical_edges)
					zafl.sh $progpath $protected --rida --tempdir $temp_dir --untracer -c all > test_${prog}.ps.log 2>&1
				;;
				zafl_untracer_critical_edges_fix_map)
					zafl.sh $progpath $protected -m --rida --tempdir $temp_dir --untracer -c all > test_${prog}.ps.log 2>&1
				;;
				zafl_rida)
