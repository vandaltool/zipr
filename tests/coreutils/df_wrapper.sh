...
touch empty.txt
timeout 2 $1  2>error.txt >/dev/null
if [ ! $? -eq 0 ]; then
...
#df does not have usage since no arguments is not specified for that purpose
...
test_transform misc/df-P
test_transform misc/df
test_transform df/total-verify
