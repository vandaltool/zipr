...
void usage(char *cmd)
{
	fprintf(stderr, "%s <target> <function_name> <address> [--malloc <address_malloc>]\n\n", cmd);
	fprintf(stderr, "example: %s a.ncexe.infern malloc 0x8043f000\n", cmd);
	fprintf(stderr, "example: %s a.ncexe.infern free 0x8043f000 --malloc 0x8042b00\n", cmd);
	exit(EXIT_CODE_TEST_INVALID);
}

void get_known_addresses(int argc, char **argv)
{
	int i;
	for (i = 0; i < argc; ++i)
	{
		if (strcmp(argv[i], "--malloc") == 0)
		{
			i++;
			if (i < argc)
				malloc_address = strtol(argv[i], NULL, 16);
...
int main(int argc, char **argv){
 ...
 /* We don't want to block any other signals in this example */
	...
	char *target = argv[1];
	char *fn = argv[2];
	uintptr_t address = strtol(argv[3], NULL, 16);
	...
	switch(pid=fork()) {
		...
		case 0 : // this is the code the child runs 
			...
			execl(target, basename(target), (char*)0);
			break;
